...
class Sitemap extends AbstractModel
{
    ...
    public function beforeSave()
    {
        ...
        if (!$this->ioFile->allowedPath($realPath, $this->directoryList->getRoot())) {
            ...
            $this->messageManager->addErrorMessage(
                __('Please create the specified folder ' . $this->getPreparedFilename() . ' before saving the sitemap.')
            );
        } elseif (!$this->ioFile->isWriteable($realPath)) {
            $this->messageManager->addErrorMessage(
                __('Please make sure that ' . $this->getPreparedFilename() . ' is writable by web-server.')
            );
        }
    ...
    protected function getPath()
    {
        if (is_null($this->_filePath)) {
            ...
            if ($dirname['dirname'] == '.') {
                $this->_filePath = str_replace('//', '/', $this->directoryList->getRoot() . '/');
            } else {
    ...
    {
        $realPath = $this->ioFile->getCleanPath($this->_getPath());
    ...
    protected function _generateProducts()
    {
        ...
        $productCollection->setStoreId($this->getStoreId())->addUrlRewrite();
    ...
    protected function _generateBrands()
    {
        ...
        foreach ($brandPages->getSource()->getAllOptions() as $option) {
            if ($option['value']) {
                $url = $shopByHelper->getSettingByValue(
                    $option['value'],
                    \Amasty\Shopby\Helper\FilterSetting::ATTR_PREFIX . $attrCode,
                    $this->_storeId
                )->getUrlPath();
    ...
    protected function _generateNavigation()
    {
        ...
        $collection->addFieldToFilter('url', ['neq' => '']);
        $collection->addStoreFilter($this->_storeId);
     ...
     * @return string
     */
    protected function _getBrandUrl($url, $type = XmlUrlInterface::URL_TYPE_LINK)
    {
