...
class FilterProducts extends \Magento\Catalog\Block\Product\AbstractProduct
{
    ...
    public function _getCfg($attr = null , $data = null)
    {
        ...
        if(isset($this->_config['widget'])){
            $this->_config['title_module'] = $this->_scopeConfig->getValue('home_carousels/widget'.$this->_config['widget'].'/title_module',\Magento\Store\Model\ScopeInterface::SCOPE_STORE,$this->_storeCode);
            $this->_config['product_source'] = $this->_scopeConfig->getValue('home_carousels/widget'.$this->_config['widget'].'/product_source',\Magento\Store\Model\ScopeInterface::SCOPE_STORE,$this->_storeCode);
            $this->_config['select_category'] = $this->_scopeConfig->getValue('home_carousels/widget'.$this->_config['widget'].'/select_category',\Magento\Store\Model\ScopeInterface::SCOPE_STORE,$this->_storeCode);
            $this->_config['product_limitation'] = $this->_scopeConfig->getValue('home_carousels/widget'.$this->_config['widget'].'/product_limitation',\Magento\Store\Model\ScopeInterface::SCOPE_STORE,$this->_storeCode);
        }
    ...
    private function _categoryProducts(){
        $count = $this->_getConfig('product_limitation');
        $category_id = $this->_getConfig('select_category');
        !is_array($category_id) && $category_id = preg_split('/[\s|,|;]/', $category_id, -1, PREG_SPLIT_NO_EMPTY);
        $collection = $this->_objectManager->create('\Magento\Catalog\Model\ResourceModel\Product\Collection');
        ...
        $collection->getSelect()->join(
            ['category_prod' => 'catalog_category_product'],
            'category_prod.product_id = e.entity_id and category_prod.category_id = '.$category_id[0],
            ['position']
        )
            ->order('category_prod.position ASC');
