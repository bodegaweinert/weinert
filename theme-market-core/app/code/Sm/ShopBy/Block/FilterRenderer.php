...
class FilterRenderer extends \Magento\LayeredNavigation\Block\Navigation\FilterRenderer
{
    ...
    public function getPriceRange($filter){
    	...
    	if($filter->getName()){
			...
			if (!empty($category)) {
                ...
                }elseif($filter instanceof \Magento\Catalog\Model\Layer\Filter\Price){
                    $return['min_standard'] = (float) $filter->getLayer()->getProductCollection()->getMinPrice();
                    $return['max_standard'] = (float) $filter->getLayer()->getProductCollection()->getMaxPrice();
                }
    ...
    public function isCategoryExcluded($category_id){
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $categoryFactory =  $objectManager->get('Magento\Catalog\Model\CategoryFactory');
        $category = $categoryFactory->create()->load($category_id);

        return $category->getFilterExclude();
    }
}