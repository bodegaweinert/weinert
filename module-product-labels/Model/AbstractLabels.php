...
class AbstractLabels extends \Magento\Framework\Model\AbstractModel
{
    ...
    {
        $product = $this->helper->getProduct($product->getId());
        ...
        switch ($this->getByPrice()) {
            ...
            case '1': // Special Price
                ...
                //fix para no tomar los special price que no apliquen por fecha
                if ($price > 0){
                    //fix para que si el special price es igual al regular price, no tomarlo
                    $regularPrice = $this->catalogData->getTaxPrice($product, $product->getData('price'), false);
                    if ($price == $regularPrice){
                        return false;
                    } else {
                        //comparo las fechas
                        $now = $this->date->date('Y-m-d 23:59:59');
