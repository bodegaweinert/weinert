...
class Merger
{
    ...
    public function merge($elements, $providerName, $dataScopePrefix, array $fieldRows = [])
    {
        foreach ($elements as $attributeCode => $attributeConfig) {
            $businessAttributes = array('require_invoice', 'business_name', 'cuit', 'convenio');
            if (in_array($attributeCode,$businessAttributes)){
    ...
    private function getFieldConfig(
        ...
        /* changing dataScopePrefix only for the custom attributes added */
        $customAttributesCodes = array('dni','street_number','street_floor','street_apartment', 'require_invoice', 'business_name', 'cuit', 'convenio','dob');
        ...
        $element = [
            ...
            'config' => [
                ...
                'customEntry' => isset($additionalConfig['config']['customEntry'])
                    ...
                    : null,
                'template' => isset($additionalConfig['config']['template'])
                    ? $additionalConfig['config']['template']
                    : 'Aheadworks_OneStepCheckout/form/field',
