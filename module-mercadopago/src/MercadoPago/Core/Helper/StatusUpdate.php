...
use MercadoPago\Core\Model\NotificationFactory;
    ...
    extends \Magento\Payment\Helper\Data
{
    ...
    public function formatArrayPayment($data, $payment, $logName)
    {
        ...
        if (isset($data['payer_identification_type'])) {
            $data['payer_identification_type'] .= " | " . (isset($payment['payer']['identification']['type'])?$payment['payer']['identification']['type']:'');
        } else {
            $data['payer_identification_type'] = (isset($payment['payer']['identification']['type'])?$payment['payer']['identification']['type']:'');
        }

        if (isset($data['payer_identification_number'])) {
            $data['payer_identification_number'] .= " | " . (isset($payment['payer']['identification']['number'])?$payment['payer']['identification']['number']:'');
        } else {
            $data['payer_identification_number'] = (isset($payment['payer']['identification']['number'])?$payment['payer']['identification']['number']:'');
        }
    ...
    public function setStatusOrder($payment)
    {
        ...
        $notification->setMpPaymentId($payment['id']);
        $notification->setNotification($message);
        $notification->setStatus($payment['status']);
        $notification->setApplied(0);
        $this->_notificationRepository->save($notification);
        /************************************************/

        $statusSave = $order->save();

        /* recargo la orden dps de guardar para que actualice los datos a los guardados */
        $order = $this->_coreHelper->_getOrder($payment["external_reference"]);
