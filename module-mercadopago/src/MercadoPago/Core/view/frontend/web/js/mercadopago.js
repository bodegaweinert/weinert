var MercadoPagoCustom = (function () {
    ...
    var self = {
        ...
        selectors: {
            ...
            installmentsRow: '#installments_row',
            ...
            issuerRow: '#issuer_row',
            ticketRow: '.option-ticket-row',
...
// MERCADO LOG
    ...
    function InitializeNewModule() {
        ...
        function initMercadoPagoJs() {
            ...
            /*added custom code*/
            TinyJ(self.selectors.cardNumberInput).keyup(handleCardNumberKeyUp);
            TinyJ(self.selectors.cardHolder).keyup(handleCardHolderKeyUp);
            TinyJ(self.selectors.docNumber).keyup(handleDocNumberKeyUp);
            TinyJ(self.selectors.securityCode).keyup(handleSecurityCodeKeyUp);
            TinyJ(self.selectors.cardExpirationMonth).change(handleCardExpirationMonthChange);
            TinyJ(self.selectors.cardExpYear).change(handleCardExpYearChange);
            TinyJ(self.selectors.docType).change(handleDocTypeChange);
            TinyJ(self.selectors.installments).change(handleInstallmentsChange);
            TinyJ(self.selectors.issuer).change(handleIssuerChange);
            ...
            /*custom for tickets*/
            setTimeout(function () {
                if (!jQuery('#empty_option_doc_type').length){
                    var emptyOption = jQuery('<option>',{label:'', value:'', id: 'empty_option_doc_type'});
                    emptyOption.prependTo(jQuery(self.selectors.docType));
                    jQuery(self.selectors.docType).val('');
                }

                jQuery('.option-ticket-row').click(function (event) {
                    var target = jQuery(event.currentTarget);
                    var selectedOption = jQuery('.option-ticket-row.selected');
                    ...
                    target.children('.ticket-option').children('input.radio').attr('checked', 'checked');
                    jQuery('.message-error.no-method-selected').css('display','none');
                });

                var documentEvents = jQuery._data(document, "events");
                if (documentEvents['forceReloadMP'] == undefined) {
                    jQuery(document).bind('forceReloadMP', function(){
                        Mercadopago.clearSession();
                        MercadoPagoCustom.getInstance().init();
                        Mercadopago.createToken(TinyJ(MercadoPagoCustom.getSelectors().customCard).getElem(), MercadoPagoCustom.getInstance().sdkResponseHandler);
                        MercadoPagoCustom.getInstance().hideMessageError();
                        MercadoPagoCustom.getInstance().hideMageError();
                        jQuery('#cardNumber').val('').trigger('change');
                        jQuery('#installments').val('').trigger('change');
        ...
        function handleCardNumberKeyUp(){
            clearMageErrorMsg(self.selectors.cardNumberInput);
        }
        function handleCardHolderKeyUp(){
            clearMageErrorMsg(self.selectors.cardHolder);
        }
        function handleDocNumberKeyUp(){
            clearMageErrorMsg(self.selectors.docNumber);
        }
        function handleSecurityCodeKeyUp(){
            clearMageErrorMsg(self.selectors.securityCode);
        }
        function handleCardExpirationMonthChange(){
            clearMageErrorMsg(self.selectors.cardExpirationMonth);
        }
        function handleCardExpYearChange(){
            clearMageErrorMsg(self.selectors.cardExpYear);
        }
        function handleInstallmentsChange(){
            clearMageErrorMsg(self.selectors.installments);
        }
        function handleIssuerChange(){
            clearMageErrorMsg(self.selectors.issuer);
        }
        function handleDocTypeChange(){
            clearMageErrorMsg(self.selectors.docType);
        }

        function clearMageErrorMsg(selector){
        ...
        function clearOptions() {
            ...
            if (bin != undefined && (bin.length == 0 || TinyJ(self.selectors.cardNumberInput).val() == '')) {
                ...
                var installmentsRow = TinyJ(self.selectors.installmentsRow);
                installmentsRow.hide();

                var issuerRow = TinyJ(self.selectors.issuerRow);
                issuerRow.hide();
        ...
        function guessingPaymentMethod(event) {
            ...
            if (event.type == self.constants.keyup) {
                if (bin != undefined && bin.length == 6) {
                    ...
                    TinyJ(self.selectors.cardNumberInput).getElem().style.background = 'none';
                }
            } else {
                setTimeout(function () {
                    if (bin != undefined && bin.length >= 6) {
            ...
            if (status == http.status.OK && response != undefined) {
                ...
                if (response.length > 1){
                    for (i=0;i< response.length; i++){
                        if (response[i].id == 'mercadopago_cc'){
                            index = i;
                        }
                    }
                    var paymentMethodId = response[index].id;
                ...
                if (response.length >1){
                    if(TinyJ(selector).getElem().value !== '') {
                        TinyJ(selector).getElem().style.background = String.format(self.constants.backgroundUrlFormat, response[index].secure_thumbnail);
                    }
                } else if (response.length == 1) {
                    if(TinyJ(selector).getElem().value !== '') {
                ...
                if (response.length > 1) {
                    additionalInfo = response[index].additional_info_needed;
                } else {
                ...
                if (issuerMandatory) {
                    if (paymentMethodId != '') {
                        ...
                        issuerRow.getElem().style.display = 'flex';
                    }
                } else {
                    TinyJ(self.selectors.issuerMp).hide();
                    issuer.hide();
                ...
                var installmentsRow = TinyJ(self.selectors.installmentsRow);
                installmentsRow.getElem().style.display = 'flex';
        ...
        function setInstallmentInfo(status, response) {
            ...
            if (response.length > 0) {
                ...
                //test line
                jQuery(option).attr('tea','TEA: 0,00%').attr('cft','CFT: 0,00%');
                //*********
        ...
        function hideMessageError(selector) {
            ...
            if (Array.isArray(allMessageErrors)) {
                for (var x = 0; x < allMessageErrors.length; x++) {
                    ...
                    jQuery(selector).siblings('input').removeClass('input-with-error');
                }
            } else {
        ...
        function showMessageErrorForm(error) {
            ...
            if (Array.isArray(messageText)) {
                for (var x = 0; x < messageText.length; x++) {
                    messageText[x].show();
                    jQuery(error).siblings('input').addClass('input-with-error');
                }
            } else {
                messageText.show();
                jQuery(error).siblings('input').addClass('input-with-error');
