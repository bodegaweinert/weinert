        ...
        return Component.extend({
            ...
            initApp: function () {
                ...
                if (window.checkoutConfig.payment[this.getCode()] != undefined) {
                    ...
                    var removeErrorMsg = this.removeErrorMessage;
            ...
            getInitialGrandTotal: function () {
                ...
                var segments = quote.totals().total_segments;
                for(var i=0;i < segments.length; i++){
                    if (segments[i]['code'] == 'factura_a_fee'){
                        initialTotal+= segments[i]['value'];
                    }
                }
            ...
            validate: function () {
                ...
                result*= $('#cardNumber').valid();

                if ($('#issuer_row').css('display') != 'none'){
                    result*= $('#issuer').valid();
                }

                result*= $('#cardholderName').valid();
                result*= $('#docType').valid();
                result*= $('#docNumber').valid();
                result*= $('#cardExpirationMonth').valid();
                result*= $('#cardExpirationYear').valid();
                result*= $('#securityCode').valid();

                if ($('#installments_row').css('display') != 'none'){
                    result*= $('#installments').valid();
                }
            ...
            hasErrors: function () {
                var allMessageErrors = jQuery('div.message-error');
