    ...
    extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    {
        $payerExceptionMessage = "Tuvimos un error en el sistema y no realizamos tu pago. Refrescá la página y volvé a intentar.";
        $payerException = new \MercadoPago\Core\Model\Api\V1\Exception(new \Magento\Framework\Phrase($payerExceptionMessage), $this->_scopeConfig);
        if ((isset($preference['payment_method']) && $preference['payment_method'] == 'credit_card') && (
            !isset($preference['payer']['identification']['type']) ||
            !isset($preference['payer']['identification']['number']) ||
            $preference['payer']['identification']['type'] == null ||
            $preference['payer']['identification']['number'] == null)
        ){

            $this->_coreHelper->log('SIN DATOS EN EL PAYER', 'mercadopago-custom.log', $preference);

            throw $payerException;
        }

        unset($preference['payment_method']);
        ...
        if (($response['status'] == 200 || $response['status'] == 201)
            && ($response['response']['status'] == 'approved' || $response['response']['status'] == 'in_process' || $response['response']['payment_type_id'] == 'ticket' || $response['response']['payment_type_id'] == 'atm')) {
            return $response;
        } else {
            $e = "";
            ...
            if ($response['status'] == 200 || $response['status'] == 201) {
                ...
                $e = $response['response']['status_detail'];

                $exception->setPhrase(new \Magento\Framework\Phrase(__($e)));
                throw $exception;
    ...
    public function getAmount($quote = null)
    {
        ...
        $total+= $quote->getShippingAddress()->getFacturaAFeeAmount();
        return (float)$total;
