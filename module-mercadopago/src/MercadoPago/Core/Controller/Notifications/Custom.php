...

{
    ...
    public function execute()
    {
        ...
        if (!empty($dataId) && $type == 'payment') {
            $this->coreHelper->log("Entra a buscar el pago en MP", self::LOG_NAME);
            ...
            if ($response['status'] == 200 || $response['status'] == 201) {
                ...
                /*aguardo unos segundos antes de ir a buscar el modelo de la order*/
                $waitingTime = $this->coreHelper->getNotificationWaitingTime();
                if ($waitingTime){
                    sleep($waitingTime);
                }
                ...
                if (!$this->_orderExists($payment['external_reference']) || $this->_order->getStatus() == 'canceled') {
                    return;
                }
    ...
    protected function _orderExists($orderID)
    {
        ...
        $this->coreHelper->log("Orden ".$orderID." no encontrada en el sitio", self::LOG_NAME);
