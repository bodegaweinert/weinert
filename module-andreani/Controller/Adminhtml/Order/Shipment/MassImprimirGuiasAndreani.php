    ...
    protected function massAction(AbstractCollection $collection)
    {
        ...
        if (count($where) == 0){
            ...
            if ($helper->getGeneracionGuiasConfig() == '1') {
                ...
                $data = $this->_prepareGuiaByItem($collection);
            }
        }
        ...
        if (count($collection->getAllIds()))
        {
            ...
            foreach ($ordersIds as $orderId)
            {
                ...
                if(!$order->hasShipments() || $leftShipments)
                {
                    try {
                        ...
                        $this->messageManager->addNoticeMessage(__('Error al generar guía de la orden ' . $order->getIncrementId() . ', por favor vuelva a intentar en unos minutos.'));
            ...
            try
            {
                ...
                if (!empty($guiasContent)) {
                    return $this->_generarGuiasMasivas($guiasContent,$collection->getAllIds());
                }
                //return true;
            }
            catch (Exception $e)
