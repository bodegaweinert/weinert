...
class Cotizar extends Action
{
    ...
    public function execute()
    {
        ...
        if($request->isXmlHttpRequest())
        {
            ...
            $sucursal = explode(',',$request->getParam('sucursalTxt'));
            $codigoPostal = intval($sucursal[1]);
            $provinciaOrigen = $request->getParam('provinciaOrigen');
            ...
            else
            {
                ...
                if($costoEnvio)
                {
                    if($helper->getProvinciasRecargo() && $helper->getProvinciasRecargoImporte() > 0){
                        if($helper->getClienteRecargo($provinciaOrigen)) {
