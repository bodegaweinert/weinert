...
class Collection extends \Magento\Sales\Model\ResourceModel\Order\Grid\Collection implements SearchResultInterface
{
    ...
    protected function _getFilteredOrders(){
        ...
        $orders = $this->_conn->fetchAll(
            'SELECT entity_id 
                  FROM sales_order 
                  WHERE (shipping_method = "andreanisucursal_sucursal" OR shipping_method = "andreaniestandar_estandar")
                  AND status IN ("'.Workflow::STATUS_PAYMENT_ACCREDITED.'","'.Workflow::STATUS_SHIPPING_PENDING.'","contabilium_accepted");'
        );
