...
class Webservice extends WsseAuthHeader
{
    ...
    public function GenerarEnviosDeEntregaYRetiroConDatosDeImpresion($params,$carrier)
    {
        ...
        try
        {
            ...
            if (strlen($params['detalledeproductosaentregar']) > 60) {
                $params['detalledeproductosaentregar'] = substr($params['detalledeproductosaentregar'], 0, 57).'...';
                $params['detalledeproductosaretirar'] = $params['detalledeproductosaentregar'];
            }
     ...
     * @return array
     */
    public function ImpresionDeConstancia($nroAndreani)
    {
        $helper     = $this->_andreaniHelper;
        $contrato   = '';
        $dataGuia   = $this->_dataGuia;
        ...
        try
        {
            if($helper->getModo() == $helper::ENVMODTEST)
            {
                $urlGenerarEnvio  = $helper->getWSMethodUrl($helper::CONFIRCOMPRA,$helper::ENVMODTEST);
                $soapVersion      = $helper->getSoapVersion($helper::CONFIRCOMPRA,$helper::ENVMODTEST);
            }
            else
            {
                $urlGenerarEnvio  = $helper->getWSMethodUrl($helper::CONFIRCOMPRA,$helper::ENVMODPROD);
                $soapVersion      = $helper->getSoapVersion($helper::CONFIRCOMPRA,$helper::ENVMODPROD);
            }

            $this->_options['soap_version'] = 1;
            $client = new SoapClient($urlGenerarEnvio, $this->_options);
            $client->__setSoapHeaders([$this]);

            $phpresponse = $client->ImprimirConstancia(
                [
                    'entities' =>
                        [
                            'ParamImprimirConstancia'   =>
                            [
            ...
            if(isset($phpresponse->ImprimirConstanciaResult->ResultadoImprimirConstancia->PdfLinkFile))
            {
                if($helper->getDebugHabilitado())
                {
                    $helper->log(print_r($nroAndreani,true),'generacion_guia_andreani_'.date('Y_m').'.log');
                    $helper->log(print_r($phpresponse,true),'generacion_guia_andreani_'.date('Y_m').'.log');
                }

                return $phpresponse->ImprimirConstanciaResult->ResultadoImprimirConstancia->PdfLinkFile;
            }
            else
            {
                return false;
