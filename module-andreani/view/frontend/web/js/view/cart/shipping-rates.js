    ...
    function (
        ...
        return Component.extend({
            ...
            selectShippingMethod: function (methodData) {
                ...
                if(methodData.carrier_code == 'andreanisucursal' && methodData.method_code == 'sucursal')
                {
                    $('#andreanisucursal-sucursal').show();

                    // $('#andreanisucursal-provincia').val('');
                    $('#andreanisucursal-localidad').trigger("change");
                ...
                $.ajax('/andreani/webservice/cotizar',
                    {
                        ...
                        success : function (response)
                        {
                            ...
                            else
                            {
                                ...
                                $('div#andreanisucursal-price label').html('Envío a sucursal ' + response.cotizacion);
                                $('#block-shipping').removeClass('andreani-loader');
                                $('#andreanisucursal-price').show();

                                this.method_title = response.method_title;
                                this.price_incl_tax = response.cotizacion.slice(1);
                                this.amount = response.cotizacion.slice(1);
                                this.base_amount = response.cotizacion.slice(1);
                                this.price_excl_tax = response.cotizacion.slice(1);
